...
export const TournamentShortListProfilesContainer: FC<Props> = ({ divisionLevel }) => {
		...
		<Flex direction="column" gap="12px">
			<Box>
				...
				>
					...
					<Text size="body14/24" color="rgba(255, 255, 255, 0.8)" pr={{ base: '0', lg: '56px' }}>
						Прирост УС
					</Text>
				</SimpleGrid>
				{divisionLevel.leaders.map((item) => (
					<Flex
						...
						onClick={goToProfileId(item.employeeNumber)}
					>
						<PlaceInRating reward={item.reward} placeInRating={item.placeInRating} />

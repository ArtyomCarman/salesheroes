...
export const RatingListContainer = () => {
  ...
  const divisionLevelParamsValue = searchParams.get(
    "divisionLevel"
  ) as DivisionLevel;
  const isDivisionLevelParamValid = Object.values(DivisionLevel).includes(
  ...
  const isTimePeriodParamValid = Object.values(TimePeriod).includes(
    timePeriodParamsValue
  );
  const timePeriod = isTimePeriodParamValid
    ? timePeriodParamsValue
    : TimePeriod["Текущий квартал"];

  if (
    ratingList.leaders.length === 0 &&
  ...
  return (
    <>
      <Flex
        ...
        margin="0 -16px"
      >
        <Filters filters={filtersDivisionLevel} searchParam="divisionLevel" />

					...
					<Flex>
						{leadersList.map((leader, index) => (
							<Box
								...
								sx={{
									'&:not(:first-of-type)': {
										WebkitMaskImage: 'radial-gradient(circle at -16px center, #0000 24px, #000 0)',
									},
								}}
							>
								{showMoreLeaders && index === news.leaders.length - 1 ? (
										...
										content={news.leaders.map((item, itemIndex) => (
											...
											>
												...
												<Box
													position="relative"
													_before={{
														content: !(itemIndex === news.leaders.length - 1) && "''",
														position: 'absolute',
														bottom: '-11px',
														width: '100%',
														height: '1px',
									...
									>
										...
										>
											<Text color="rgba(255, 255, 255, 0.8)" size="body14/24" variant="semibold">
						...
						{news.leaders.map((leader) => (
							...
							>
								...
								<Flex flexDirection="column" flexGrow={1}>
									...
									<Text fontSize="14px" lineHeight="24px" color="rgba(255, 255, 255, 0.8)">
